...
func TestAttestationAdapter_Verify(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"verify success": {
			...
			storeResult: func(ctx context.Context, r *Request) error { return nil },
			wantErr:     nil,
		},
		...
		"isChallengeAssigned error": {
			...
			isChallengeAssigned: func(ctx context.Context, r *Request) (bool, error) { return false, errors.New("check error") },
			wantErr:             ErrInternal,
		},
		"store result error": {
			extractData: func(ctx context.Context, r *Request) (*attest.AttestationObject, []byte, []byte, error) {
			...
			storeResult: func(ctx context.Context, r *Request) error { return errors.New("store failed") },
			wantErr:     ErrInternal,
		},
	}
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			plugin := &mockPluginFunc{
				...
				newChallenge:        func(ctx context.Context, r *Request) (string, error) { return "mock-challenge", nil },
...
func TestAttestationAdapter_NewChallenge(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"plugin error": {
			newChallenge: func(ctx context.Context, r *Request) (string, error) { return "", errors.New("plugin failed") },
			wantErr:      ErrInternal,
		},
	}
